...
void MainLight_float(float3 WorldPos, out float3 Direction, out float3 Color, out float DistanceAtten, out float ShadowAtten)
{
    ...
    float shadowParams = GetMainLightShadowParams();
    ShadowAtten = SampleShadowmapFiltered(TEXTURE2D_SHADOW_ARGS(_MainLightShadowmapTexture, sampler_MainLightShadowmapTexture), shadowCoord, shadowSamplingData);
    ...
    //OLD ShadowAtten = SampleShadowmap(shadowCoord, TEXTURE2D_ARGS(_MainLightShadowmapTexture, sampler_MainLightShadowmapTexture), shadowSamplingData, shadowStrength, shadowParams, false);
#endif
...
void AdditionalLightsCartJam_float(float3 WorldPosition, float3 WorldNormal, out float Luminance)
{
    float luminance = 0;
    float3 diffuseColor = 0;
    WorldNormal = normalize(WorldNormal);
    ...
    {
        Light light = GetAdditionalLight(i, WorldPosition);
        luminance += light.distanceAttenuation * light.shadowAttenuation;
        half3 attenuatedLightColor = light.color * (light.distanceAttenuation * light.shadowAttenuation);
        diffuseColor += LightingLambert(attenuatedLightColor, light.direction, WorldNormal);
    }
#endif

    Luminance = length(diffuseColor); 
}
